...
def im_detect_yolo(yolo_net, im):
  ...
  for bb in predict:
    if bb["label"] in ["car", "motorbike"]:
      cls = VOC_CLASSES.index(bb["label"])
    else:
      continue
    box_with_score = [bb["topleft"]["x"], bb["topleft"]["y"], bb["bottomright"]["x"], bb["bottomright"]["y"],
                      bb["confidence"]]
    all_boxes[cls].append(box_with_score)

  for cls in range(21):
    all_boxes[cls] = np.asarray(all_boxes[cls])
  ...
  if not has_boxes:
    for i in range(num_images):
      ...
      if net_structure == 'faster-rcnn':
        ...
        _t['im_detect'].tic()
        new_bboxes = im_detect_yolo(net, im)
        _t['im_detect'].toc()
        _t['misc'].tic()
        for j in range(1, imdb.num_classes):
          all_boxes[j][i] = new_bboxes[j]
        _t['misc'].toc()
